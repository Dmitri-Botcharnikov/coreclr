...
ModuleInfo::ModuleInfo( ModuleID moduleID, SIZE_T internal ) :
    ...
    m_loadAddress( 0 )
{
    wcscpy_s( m_moduleName, ARRAY_LEN(m_moduleName), W("UNKNOWN") );
...
{
    wcscpy_s( m_className, ARRAY_LEN(m_className), W("UNKNOWN") );
...
//                Failure( "IProfilerInfo::GetModuleMetaData() => IMetaDataImport FAILED" );
                wcscpy_s(className, MAX_LENGTH, W("???"));
                hr = S_OK;
            }
        }
        else
        ...
        *backTick = L'\0';
        ...
        if (genericArgCount >0)
        {
            ...
            *totalGenericArgCount += genericArgCount;
            ...
            _snwprintf_s(className, MAX_LENGTH, MAX_LENGTH-1, W("%s%S"), className, typeArgText);
        }
    }
    return hr;
}
                                        ...
                                        size_t functionNameLen )
{
    ...
    if ( functionID != NULL )
    {
        ...
        IMetaDataImport *pMDImport = NULL;
        WCHAR funName[MAX_LENGTH] = W("UNKNOWN");
        ...
        if ( SUCCEEDED( hr ) )
        {
            ...
            if ( SUCCEEDED( hr ) )
            {
                WCHAR className[MAX_LENGTH] = W("UNKNOWN");
                ClassID classId =0;
                ...
                _snwprintf_s( functionName, functionNameLen, functionNameLen-1, W("%s::%s"), className, funName );
                ...
                {
                    static const char* callConvNames[8] =
                    {
                        "",
                        "unmanaged cdecl ",
                        "unmanaged stdcall ",
                    ...
                    _snwprintf_s( returnTypeStr, returnTypeStrLen, returnTypeStrLen-1, W("%S"),buffer );
                          ...
                          i++ )
                    {
                        ...
                        if ( i == 0 )
                        {
                            _snwprintf_s( functionParameters, functionParametersLen, functionParametersLen-1, W("%S"), buffer );
                        }
                        else if ( sigBlob != NULL )
                        {
                            _snwprintf_s( functionParameters, functionParametersLen, functionParametersLen-1, W("%s+%S"), functionParameters, buffer );
                        }
                ...
                else
                {
                    ...
                    _snwprintf_s( returnTypeStr, returnTypeStrLen, returnTypeStrLen-1, W("%s %S"),returnTypeStr, buffer );
                }
            }
    ...
    else
    {
        ...
        *argCount = 0;
        *isStatic = FALSE;
        wcscpy_s(functionName, functionNameLen, W("UNMANAGED FRAME") );
    }
